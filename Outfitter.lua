...
-- Aspect of the Hawk and Aspect of the Dragonhawk used to map onto a "Hawk"
-- special outfit here.  Both aspects were removed from the game in Legion and the
-- hunter entry in cClassSpecialOutfits was emptied, so nothing could match or act
-- on them; the spell IDs are kept in Deprecated.HawkSpellIDs
...
function Outfitter:OnLoad()
	...
	-- Outfitter.xml builds the window inside OutfitterButtonFrame, which is parented
	-- to PaperDollFrame.  Every list row owns a secure button, and a protected frame
	-- restricts both the frames it's parented to and the frames it's anchored to, so
	-- leaving the window there makes the character sheet unmovable for anything else
	-- that lays it out.  The buttons themselves can't be detached (their anchors are
	-- unrepairable, see _ListItem:disableSecureActions), so the window moves instead

	self:DetachWindow()

	OutfitterFrame:SetFrameStrata("HIGH")

	-- It used to inherit its visibility from PaperDollFrame; now it has to be told

	PaperDollFrame:HookScript("OnHide", function ()
		if OutfitterFrame:IsShown() then
			OutfitterFrame:Hide()
		end
	end)

	OutfitterFrame:HookScript("OnUpdate", function (frame, elapsed)
		self:WindowFollowUpdate(elapsed)
	end)
end

-- Anchor the window to the right of the character sheet, without linking the two
-- frames together: the offsets are resolved against UIParent, so nothing in the
-- character sheet's layout depends on this window or on the secure buttons in it.
--
-- The single TOPLEFT SetPoint replaces the one Outfitter.xml set, so there's no
-- ClearAllPoints to undo if the client rejects the call

function Outfitter:PositionWindow()
	local frame = OutfitterFrame
	local right, top = PaperDollFrame:GetRight(), PaperDollFrame:GetTop()

	if not right or not top then
		-- The character sheet has never been laid out, so there's nothing to line up
		-- with yet.  Park it; it gets positioned properly when it's opened

		self.WindowFollowRight, self.WindowFollowTop = nil, nil
		frame:SetPoint("TOPLEFT", UIParent, "TOPLEFT", 0, 0)
		return
	end

	local scale = frame:GetEffectiveScale()
	local anchorScale = PaperDollFrame:GetEffectiveScale()

	self.WindowFollowRight, self.WindowFollowTop = right, top

	frame:SetPoint("TOPLEFT", UIParent, "BOTTOMLEFT",
		right * anchorScale / scale,
		top * anchorScale / scale - 30)
end

-- A real anchor to the character sheet is what breaks other addons, so the window
-- follows it instead: while it's open, watch the corner it lines up with and
-- re-issue the UIParent-relative SetPoint whenever it moves.  Dragging the sheet,
-- the panel manager shuffling it sideways and UI scale changes all show up here.
--
-- Only runs while the window is visible, and only touches SetPoint when the corner
-- has actually moved

function Outfitter:WindowFollowUpdate(pElapsed)
	if self.WindowFollowDisabled then
		return
	end

	self.WindowFollowElapsed = (self.WindowFollowElapsed or 0) + pElapsed

	if self.WindowFollowElapsed < 0.05 then
		return
	end

	self.WindowFollowElapsed = 0
	...
	if right == self.WindowFollowRight and top == self.WindowFollowTop then
		return
	end
...
function Outfitter:DetachWindow()
	if InCombatLockdown() then
		return
	end

	if OutfitterFrame:GetParent() ~= UIParent then
		OutfitterFrame:SetParent(UIParent)
	end

	self:AttachWindow()
end

function Outfitter:AttachWindow()
	if InCombatLockdown() then
		return
	end

	if pcall(self.PositionWindow, self) then
		return
	end

	-- The client wouldn't let us move the window either, because of the secure
	-- buttons inside it.  Fall back to the layout Outfitter.xml describes so the
	-- window is at least usable; other addons then can't reposition the character
	-- sheet while it's parented back in.  It's a real anchor again at that point, so
	-- there's nothing left for the follow to do

	self.WindowFollowDisabled = true

	OutfitterFrame:SetParent(OutfitterButtonFrame)
end

function Outfitter:OnShow()
	self:AttachWindow()

	self.SetFrameLevel(OutfitterFrame, OutfitterFrame:GetFrameLevel())
...
function Outfitter:PlayerIsFull()
	...
	-- of those is an error.  There's no way to tell how full the player is without
	...
	if not vHealth or not vHealthMax or vHealthMax == 0 then
		return false
	end

	if vHealth < (vHealthMax * 0.85) then
		return false
	end
...
function Outfitter:ShowCommandHelp()
	...
	self:NoteMessage(HIGHLIGHT_FONT_COLOR_CODE.."/outfitter summary"..NORMAL_FONT_COLOR_CODE..": Summarizes your outfits and the items they use")
	self:NoteMessage(HIGHLIGHT_FONT_COLOR_CODE.."/outfitter rating"..NORMAL_FONT_COLOR_CODE..": Summarizes the combat ratings of your outfits")
	self:NoteMessage(HIGHLIGHT_FONT_COLOR_CODE.."/outfitter iteminfo itemLink"..NORMAL_FONT_COLOR_CODE..": Reports what Outfitter knows about an item")
	self:NoteMessage(HIGHLIGHT_FONT_COLOR_CODE.."/outfitter itemstats itemLink"..NORMAL_FONT_COLOR_CODE..": Reports the parsed stats of an item")
...
function Outfitter:Update(pOutfitsChanged)
	...
	if self.CurrentPanel == 3 then
		...
		for vItemIndex2 = vItemIndex, (self.cMaxDisplayedItems - 1) do
			...
			-- Hide the secure button before the row goes away so nothing is left
			-- clickable pointing at whatever item the row held last
...
function Outfitter:SelectOutfit(pOutfit)
	...
	-- The checkboxes sit on top of the character sheet's item buttons, and the frame
	-- inherits PaperDollFrame's strata, so anything else drawing over the character
	-- panel at a higher strata buries them.  Raise it here rather than in OnLoad:
	-- OutfitterFrame's OnLoad runs before Outfitter.xml has created this frame

	OutfitterSlotEnables:SetFrameStrata("DIALOG")
...
function Outfitter:Initialize()
	...
	if tonumber(C_AddOns.GetAddOnMetadata("Outfitter", "X-ReloadTag")) ~= 2 then
		...
		OutfitterFrame:Hide() -- no longer a child of the button frame, so hide it too
...
function Outfitter:GetPlayerStat(pStatIndex)
	...
	-- Arithmetic on a secret value is an error, so report the stat as zero when the
	-- client won't hand over real numbers

	vEffectiveValue = OutfitterAPI:UnsecretNumber(vEffectiveValue, 0)
	vPosValue = OutfitterAPI:UnsecretNumber(vPosValue, 0)
	vNegValue = OutfitterAPI:UnsecretNumber(vNegValue, 0)
...
-- original implementation is Deprecated.VoidStorage.DepositOutfit

function Outfitter:DepositOutfitToVoidStorage(pOutfit, pUniqueItemsOnly)
	self:DeprecatedFeature("Void storage deposit")
end

-- Disabled for the same reason, and it has no implementation to disable: the
-- WITHDRAWVOID outfit menu action has always called a method that was never
-- written, so picking it raised "attempt to call a nil value"

function Outfitter:WithdrawOutfitFromVoidStorage(pOutfit)
	self:DeprecatedFeature("Void storage withdraw")
end
...
-- Disabled: GetNumCompanions and CallCompanion went away when pets moved into the
-- pet journal.  Reachable from user scripts, so the method itself stays.  The
...
function Outfitter._ListItem:disableSecureActions()
	...
	-- Hiding it is all we can do.  Never re-parent it and never touch its anchors:
	-- the client refuses every SetPoint an insecure caller makes on a protected
	-- frame ("Cannot anchor protected frames to regions"), so the setAllPoints
	-- anchor Outfitter.xml creates at load is the only one this button will ever
	-- have, and dropping it can't be undone.  The character sheet is kept clear of
	-- these buttons by moving the whole window instead -- see Outfitter:DetachWindow
...
function Outfitter._ListItem:enableSecureActions()
	...
	-- Same again: parent and anchors come from Outfitter.xml and stay untouched.
	-- The button has to be a real secure button covering the row rather than the row
	-- itself, because item-targeting spells such as Disenchant need the click to
	-- land on a secure frame; UseContainerItem is protected
...
function Outfitter._ListItem:OnEnter()
	...
	elseif self.isOutfitItem then
		...
		elseif self.outfitItem.Location.BagIndex == -1 then
			-- Disabled: BankButtonIDToInvSlotID was removed with the old bank frame,
			-- so there's no way to reach a banked item's tooltip.  Original in
			-- Deprecated.BankTooltip.SetItem

			Outfitter:DeprecatedFeature("Banked item tooltip")
		else
