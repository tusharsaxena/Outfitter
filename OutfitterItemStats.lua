...
function Outfitter._PawnScaleStat:GetItemScore(pItem)
	...
	if not vScore then
		...
		-- Ignore the item if it's too high level.  Same as above: without a usable
		-- level, score the item normally rather than zeroing it
		elseif vPlayerLevel and vStats.minLevel and vStats.minLevel > vPlayerLevel then
			vScore = 0
...
-- Disabled and moved to Deprecated.lua (Deprecated.TankPoints).  It had stopped
-- working long before it was retired: Outfitter.Stats_AddStatValue was never
-- defined anywhere in the addon, UnitDefense was removed from the game, and the
-- block that registered the stat category was already commented out.
--
-- Nothing referenced Outfitter.TankPointsCategory or Outfitter.TankPoints_* from
-- outside this file, so there is no entry point left to stub
